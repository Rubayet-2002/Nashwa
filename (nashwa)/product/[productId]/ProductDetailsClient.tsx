...
export default function ProductDetailsClient({
  ...
  useEffect(() => {
    ...
    fetch(`/api/shops/${product.shop_uid}`)
      ...
      .then((d) => {
        if (active && d.posts) {
          const filtered = d.posts.filter((p: any) => p.product_uid !== product.product_uid);
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              {activeTab === "reviews" && (
                <div className="flex flex-col gap-4">
                  {loadingReviews ? (
                    ...
                    <div className="flex flex-col gap-4 divide-y divide-gray-150">
                      {reviews.map((r) => (
                        <div key={r.review_uid} className="pt-4 first:pt-0 flex flex-col gap-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-xs font-bold text-[#1a1a1a]">{r.username}</p>
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
          ...
          {/* Shop Card */}
          <div className="bg-white border border-[#e8e8e8] rounded-3xl p-5 shadow-xs flex flex-col gap-4">
            <h3 className="text-xs font-bold text-[#BA5B55] uppercase tracking-wider">Merchant Partner</h3>
            
            <div className="flex items-center gap-3">
              <Link href={`/shop/${product.shop_uid}`}>
                <div className="relative w-12 h-12 rounded-full overflow-hidden border border-[#e8e8e8] bg-[#fafafa] flex items-center justify-center shrink-0 shadow-3xs">
                  {product.shop_profile_photo_url ? (
                    <Image src={product.shop_profile_photo_url} alt={product.shop_name} fill className="object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[#BA5B55] font-bold text-sm uppercase">
              ...
              <div className="min-w-0">
                ...
                <span className="text-[10px] text-[#787878] leading-none block mt-1.5">
                  {product.shop_university_name || product.shop_location}
                </span>
              </div>
            </div>

            {/* Shop follow / Contact widgets */}
          ...
          {/* More products list */}
          <div className="bg-white border border-[#e8e8e8] rounded-3xl p-5 shadow-xs flex flex-col gap-4">
            <h3 className="text-xs font-bold text-[#1a1a1a] uppercase tracking-wider">More from this shop</h3>
            ...
            {loadingMoreProducts ? (
              ...
              <div className="flex flex-col gap-3">
                {moreProducts.map((p) => (
                  <Link
                    key={p.product_uid}
                    href={`/product/${p.product_uid}`}
                    className="flex items-center gap-3 p-2 rounded-2xl border border-gray-50 hover:border-[#BA5B55]/20 hover:bg-[#fafafa]/50 transition-all"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <h4 className="text-xs font-bold text-[#1a1a1a] truncate leading-none">{p.title}</h4>
                      <p className="text-xs font-semibold text-[#BA5B55] mt-1.5">৳{Number(p.price).toFixed(0)}</p>
                    </div>
                  </Link>
                ))}
              </div>
