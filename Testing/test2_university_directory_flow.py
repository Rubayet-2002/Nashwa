...
def test_university_directory_flow():
    driver = webdriver.Edge(options=options, service=service_obj)
    try:
        login(driver)
        driver.get(f"{BASE_URL}/university")

        WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.XPATH, "//h1[contains(., 'Universities') or contains(., 'University')]"))
        )
