...
def test_university_shop_page_flow():
    driver = webdriver.Edge(options=options, service=service_obj)
    try:
        login(driver)
        driver.get(f"{BASE_URL}/university/uiu")

        header = WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.XPATH, "//h1 | //h2 | //p[contains(., 'University shops directory')]"))
        )
