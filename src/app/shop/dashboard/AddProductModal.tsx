...
export default function AddProductModal({
  ...
  // 8. Product Options (Specifications)
  const [options, setOptions] = useState<OptionField[]>([]);

  // 9. Product Variants (Selectable Purchase Choices)
    ...
    if (e.target.files) {
      ...
      setSelectedFiles((prev) => {
        ...
        if (combined.length > 4) {
          addToast("You can select up to 4 images maximum", "error");
        }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // 1. Upload files to Cloudinary one by one
      ...
      setUploadProgressText("Creating product listing...");
      ...
      console.error(err);
      addToast("Failed to upload images or save product due to network error", "error");
    } finally {
      setSubmitting(false);
          ...
          <div className="mt-1 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-xs text-[#787878]">
                Fill details in order and submit. Images will upload when you click Create Product.
              </p>
            </div>
            <div className="hidden rounded-none border border-[#eadfdb] bg-white px-4 py-2 text-right md:block">
              <p className="text-[10px] uppercase tracking-[0.2em] text-[#787878]">Selected Images</p>
              <p className="text-sm font-semibold text-[#1a1a1a]">{selectedFiles.length} of 4</p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <label className="text-[11px] font-extrabold uppercase tracking-wider text-[#BA5B55] flex items-center gap-1.5">
                <span>1. Select Product Photos (Max 4)</span>
                <span className="text-[#BA5B55]">*</span>
              </label>
              <span className="text-[10px] text-gray-400 font-light">Local preview shown below</span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-4 items-start">
              ...
              <div className="sm:col-span-4 grid grid-cols-4 gap-3">
                {selectedFiles.map((file, i) => (
                  <div key={i} className="relative aspect-square border border-[#eadfdb] bg-gray-50 group overflow-hidden">
                ...
                {selectedFiles.length === 0 && (
                  <div className="col-span-4 aspect-video sm:aspect-auto sm:h-20 border border-dashed border-[#eadfdb] rounded-none flex items-center justify-center text-xs text-gray-400">
                    No images selected yet. Choose up to 4 photos.
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-gray-100 pt-5">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex items-center border border-[#eadfdb] bg-white px-3 py-1.5 transition-colors focus-within:border-[#BA5B55] rounded-none">
                <span className="text-xs text-[#787878] font-bold">{currency}</span>
                <input
                  value={price}
