...
const ShopDashboard = async () => {
  ...
  const statusColor = shop?.shopStatus === "approved" ? "text-green-600" : "text-[#eab308]";
      ...
      <aside className="w-90 shrink-0 flex flex-col bg-white overflow-hidden">
        ...
        <div className="flex flex-col gap-3 p-6 flex-1">
          <div className="flex items-center gap-3 text-[#787878] text-sm">
            <Store stroke={1.5} size={16} className="shrink-0" />
            <span className="truncate font-semibold">{shop?.shopName ?? "No Shop Name"}</span>
          </div>

          <div className="flex items-center gap-3 text-[#787878] text-sm">
            <span className={`font-medium ${statusColor}`}>Status: {shop?.shopStatus ?? "pending"}</span>
          </div>
