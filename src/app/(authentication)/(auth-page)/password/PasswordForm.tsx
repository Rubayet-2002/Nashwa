...
const PasswordForm = ({ email, uid }: PasswordFormProps) => {
  ...
  const handleClientAction = async (prevState: any, formData: FormData) => {
    ...
    try {
      const response = await fetch("/api/login", {
        ...
        body: JSON.stringify({ email: email, uid: uid, password: password }),
      });
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2">
      ...
      >
        <p className="leading-none">{isPending ? "Logging in..." : "Login"}</p>
      </button>
