...
import { ChatMessages, X, ChevronsRight, Store } from "@mynaui/icons-react";
...
export default function ContactSellerWidget({
  ...
  useEffect(() => {
    ...
    const fetchChatHistory = async () => {
      try {
        const res = await fetch(`/api/chat?shopUid=${encodeURIComponent(shopUid)}`);
    ...
    <>
      ...
      {iconOnly ? (
        <button
          ...
          type="button"
          className="w-9 h-9 bg-[#BA5B55] border border-[#BA5B55] hover:bg-white hover:text-[#BA5B55] text-white transition-all rounded-none flex items-center justify-center cursor-pointer shadow-sm shrink-0"
        ...
        <button
          onClick={handleOpenChat}
          type="button"
          className="px-4 py-2 bg-[#BA5B55] border border-[#BA5B55] hover:bg-white hover:text-[#BA5B55] text-white transition-all text-xs font-semibold flex items-center gap-1.5 cursor-pointer shadow-sm rounded-none"
        >
        ...
        <div className="fixed bottom-6 right-6 z-50 w-80 sm:w-96 h-[450px] bg-white border border-[#eadfdb] flex flex-col rounded-none overflow-hidden animate-in slide-in-from-bottom-5 duration-200">
          ...
          <div className="bg-[#BA5B55] text-white px-4 py-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="relative h-8 w-8 rounded-none overflow-hidden border border-white/20 bg-white/10 flex justify-center items-center shrink-0">
                {shopAvatar ? (
          ...
          {/* Messages Stream */}
          <div className="flex-1 p-4 overflow-y-auto bg-[#fdfbf9] flex flex-col gap-3 custom-scrollbar">
            {messages.length === 0 ? (
                    ...
                    <div
                      className={`px-3 py-2 text-xs ${
                        isMe
                          ...
                          : "bg-white border border-[#e8e1df] text-[#1a1a1a] rounded-[12px_12px_12px_3px]"
                      }`}
                    >
                      {msg.message_text}
                    </div>
          ...
          <form onSubmit={handleSendMessage} className="p-3 border-t border-[#eadfdb] bg-white flex gap-2 items-center">
            <input
              ...
              disabled={loading}
              className="flex-1 text-xs border border-[#e8e1df] rounded-none px-3.5 py-2 outline-none focus:border-[#BA5B55] transition-all disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={loading || !inputText.trim()}
              className="p-2 bg-[#BA5B55] hover:bg-[#BA5B55]/90 disabled:bg-gray-200 text-white rounded-none transition-colors flex items-center justify-center cursor-pointer shrink-0"
              title="Send message"
            >
